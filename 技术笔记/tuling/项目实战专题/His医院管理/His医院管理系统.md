...
原有的系统是针对一个医院内部的信息管理系统。SAAS化，软件即平台，希望在平台上，多个医院都可以正常使用。 有一种简单的saas设计，是只分数据。在每个业务表上增加tenant_id租户ID字段，区分每个医院。 这些医院的数据时在一起的，大医院和小医院会有影响。 这里采用了一种分库的实现方式，每个医院对应后端一套独立的mysql数据库，数据隔离。
